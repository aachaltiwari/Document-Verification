...
logging.basicConfig(level=logging.INFO)  # Make sure logging is configured
...
def citizenship_validate(request):
    ...
    if request.method == 'POST':
        ...
        if front_file:
            ...
            errors.append(f"Front image required")
        # Validate back image
        if back_file:
            validate_and_collect_errors(back_file, "Back Image")
        else:
            errors.append(f"Back image required")

        # If there are any errors, return them
        if errors:
            return JsonResponse({'success': False, 'errors': errors})

        # If both validations pass, save the images
        try:
            timestamp = timezone.now().strftime("%Y%m%d_%H%M%S")
            save_directory = os.path.join(settings.MEDIA_ROOT, 'citizenship', timestamp)
            os.makedirs(save_directory, exist_ok=True)  # Ensure directory exists

            front_filename = 'front.jpg'  # Or .png, based on the file type you expect
            back_filename = 'back.jpg'

            front_filepath = os.path.join(save_directory, front_filename)
            back_filepath = os.path.join(save_directory, back_filename)

            # Save front image
            with open(front_filepath, 'wb+') as destination:
                for chunk in front_file.chunks():
                    destination.write(chunk)
            logging.info(f"Saved front image to {front_filepath}")

            # Save back image
            with open(back_filepath, 'wb+') as destination:
                for chunk in back_file.chunks():
                    destination.write(chunk)
            logging.info(f"Saved back image to {back_filepath}")

            # Return success message
            return JsonResponse({'success': True, 'message': 'Images passed validation and saved!'})

        except Exception as e:
            logging.error(f"Error saving images: {str(e)}")
            return JsonResponse({'success': False, 'errors': [f'Error saving images: {str(e)}']})  #Include more error information
    else:
